/*Track_car_V5.0 20191225 by yee11111*/
...
{Serial.println("Inital....."); int Hs=700,Ls=200,Op=100,mt=0;
  while(mt<5){
	  rtrnS(Op,ffix); Maxmin_norml_V2(); if(analogRead(Mid)>Hs) { mt=mt+1; }  //xx1xx
  } whelstop();
}
...
}else {np=((L2n*-d2*dd1)+(L1n*-d1*dd2)+(Mn*d0)+(R1n*d1*dd4)+(R2n*d2*dd5)); error=np-hop;}
}
...
{ int cc2=20,Ls=200,tt=1;
  while(cm<cc2)
  {
    take_dist_V3(); whelstop(); while(cm<25) { take_dist_V3(); rtrnS(150,ffix); }
    rtrnS(150,ffix); delay(100); font(150,ffix); delay(450); analogWrite(6,abs(200));analogWrite(5,abs(50)); delay(800);
    while(analogRead(L2)<Ls&&analogRead(L1)<Ls&&analogRead(Mid)<Ls&&analogRead(R1)<Ls&&analogRead(R2)<Ls) {font(150,ffix);}
  }
}

void fontline_V3() {int rg=1; if(error<rg&&error>-rg&&analogRead(Mid)>700){take_dist_V3(); rond_V2(); } Maxmin_norml_V2(); take_err(); crtl_V5(); }
...
void setup() {
  ...
  pinMode(trigPin, OUTPUT);  hop=0; Maxmin_norml_V2(); take_err(); 
}

void loop() {
  Maxmin_norml_V2(); 
  if(analogRead(A7)==0)
  { // init_V5(); Maxmin_norml_V2();
